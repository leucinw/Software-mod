...
gen_opts_types = {
    'strings' : {"gmxpath"      : (which('mdrun'), 60, 'Path for GROMACS executables (if not the default)', 'All targets that use GROMACS', ['GMX']),
                 ...
                 "amoeba_pol"   : (None, 0, 'The AMOEBA polarization type, either direct, mutual, OPTx, or nonpolarizable.', 'Targets in OpenMM / TINKER that use the AMOEBA force field', ['OPENMM','TINKER']),
...
tgt_opts_types = {
    'strings' : {"force_map" : ('residue', 0, 'The resolution of mapping interactions to net forces and torques for groups of atoms.  In order of resolution: molecule > residue > charge-group', 'Force Matching', 'AbInitio'),
                 ...
                 "sfedata_txt"           : ('sfedata.txt', 0, 'Text file containing experimental data.', 'Solvation free energy target', 'Solvation'),
                 "autobar_path"           : (os.getenv("AUTOBARPATH"), 0, 'absolute path of autoBAR.py program', 'Solvation free energy target', 'Solvation'),
                 "run_dynamic_every_iter"  : (1, 0, 'whether run dynamic every iteration. 0 for No', 'Solvation free energy target', 'Solvation'),
    ...
    'ints'    : {"shots"              : (-1, 0, 'Number of snapshots; defaults to all of the snapshots', 'Energy + Force Matching', 'AbInitio'),
                 ...
                 "md_threads"         : (6, 0, 'Set the number of threads used by Gromacs or TINKER processes in MD simulations', 'Condensed phase properties in GROMACS and TINKER', 'Liquid_GMX, Lipid_GMX, Liquid_TINKER'),
    ...
    'floats'  : {"weight"       : (1.0, 150, 'Weight of the target (determines its importance vs. other targets)', 'All targets (important)'),
                 ...
                 "sfe_temperature"  : (298.15, -100, 'Simulation temperature for solvation free energies (Kelvin)', 'Solvation free energy using molecular dynamics', 'solvation'),
                 "sfe_pressure"   : (1.0, -100, 'Simulation pressure for solvation free energies (atm)', 'Solvation free energy using molecular dynamics', 'solvation'),
                 ...
                 "energyscale"     : (1.0, 0, 'Scaling factor for Energy', 'Minimum Match targets', 'minimum_match'),
                 "bondscale"     : (30.0, 0, 'Scaling factor for Bond length', 'Minimum Match targets', 'minimum_match'),
                 "anglescale"     : (0.02, 0, 'Scaling factor for Angle value', 'Minimum Match targets', 'minimum_match'),
